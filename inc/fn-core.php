...
function am_add_javascript( ) {
    ...
    if( !is_admin() ) {
        $am_files = array('/assets/js/slick/slick.min.js', '/assets/js/script.js'); // example: array('script1', 'script2');
        foreach($am_files as $am_file){
            wp_enqueue_script('am_'.sanitize_title($am_file), get_theme_file_uri($am_file), array( 'jquery' ),filemtime( get_theme_file_path($am_file)) );
        }
        //external Javascript
        $am_links = array('https://cdnjs.cloudflare.com/ajax/libs/wow/1.1.2/wow.min.js', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.19.1/jquery.validate.min.js'); 
...
function am_add_css( ) {
    ...
    // internal CSS
    $am_files = array('/assets/js/slick/slick.css', '/assets/js/slick/slick-theme.css', '/assets/css/animate.css', 'style.css'); // example: array('style1', 'style2');
    foreach($am_files as $am_file){
        wp_enqueue_style('am_'.sanitize_title($am_file), get_theme_file_uri($am_file),array(),filemtime( get_theme_file_path($am_file)));
    }
    
    // external CSS
    $am_links = array('https://fonts.googleapis.com/css2?family=Roboto:wght@100;300;400&display=swap'); 
    foreach($am_links as $am_link){
